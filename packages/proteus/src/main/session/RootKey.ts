...
export class RootKey {
   ...
   */
  static fromCipherKey(cipherKey: CipherKey): RootKey {
    const rootKeyInstance = ClassUtil.newInstance(RootKey);
    rootKeyInstance.key = cipherKey;
    return rootKeyInstance;
  }
   ...
   */
  dhRatchet(ours: KeyPair, theirs: PublicKey): [RootKey, ChainKey] {
    const secret = ours.secretKey.sharedSecret(theirs);
    const derivedSecrets = DerivedSecrets.kdf(secret, this.key.key, 'dh_ratchet');

    return [RootKey.fromCipherKey(derivedSecrets.cipherKey), ChainKey.fromMacKey(derivedSecrets.macKey, 0)];
  }
  ...
  static decode(decoder: CBOR.Decoder): RootKey {
    ...
    for (let index = 0; index <= nprops - 1; index++) {
      switch (decoder.u8()) {
        case 0:
          cipherKey = CipherKey.decode(decoder);
          break;
        default:
          decoder.skip();
      }
    ...
    if (cipherKey) {
      return RootKey.fromCipherKey(cipherKey);
    } else {
