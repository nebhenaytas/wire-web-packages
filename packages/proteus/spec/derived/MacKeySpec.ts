...
describe('Mac Key', () => {
  it('encodes a message', () => {
    const keyMaterialBuffer = new ArrayBuffer(32);
    const typedKeyMaterial = new Uint8Array(keyMaterialBuffer);
    const macKey = new Proteus.derived.MacKey(typedKeyMaterial);
    const message = sodium.from_string('hello');

    const authenticationCode = macKey.sign(message);
    ...
    expect(authenticationCode).toEqual(expected);
  });
  ...
  it('verifies correct data', () => {
    ...
    // prettier-ignore
    const message = new Uint8Array([1, 165, 0, 80, 82, 255, 214, 194, 215, 199, 190, 124, 44, 155, 253, 88, 108, 182, 97, 218, 1, 0, 2, 1, 3, 161, 0, 88, 32, 81, 164, 93, 189, 218, 73, 99, 191, 236, 250, 188, 97, 212, 13, 88, 41, 115, 127, 228, 229, 168, 46, 142, 241, 211, 60, 155, 78, 219, 59, 171, 17, 4, 66, 130, 57]);
    // prettier-ignore
    ...
    const macKey = new Proteus.derived.MacKey(key);

    expect(macKey.verify(signature, message)).toBe(true);
  });
  ...
  it('verifies a calculated signature', () => {
    ...
    const signature = macKey.sign(message);
  ...
  it('verifies calculated data', async () => {
    ...
    const message = sodium.from_string('This is my great message in Proteus!');
    const signature = macKey.sign(message);
